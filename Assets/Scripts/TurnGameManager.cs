...
public class TurnGameManager: MonoBehaviour
{
    ...
    //UI element
    public static bool isParamatersSetup = false;

    [SerializeField]
    GameObject ParameterSetupPanel;
    [SerializeField]
    GameObject PausePanel;

    [SerializeField]
    Slider maxTurnNumSlider;
    [SerializeField]
    Text maxTurnNumText;

    [SerializeField]
    Slider timeForEachTurnSlider;
    [SerializeField]
    Text timeForEachTurnText;

    [SerializeField]
    Slider timeForCheckSlider;
    [SerializeField]
    Text timeForCheckText;

    void Start()
    {
        ParameterSetupPanel.SetActive(true);
        PausePanel.SetActive(false);
    }

    void FixedUpdate()
    {
        if (!isParamatersSetup)
        {
            maxTurnNumText.text = (maxTurnNumSlider.value * 2).ToString();
            timeForEachTurnText.text = timeForEachTurnSlider.value.ToString();
            timeForCheckText.text = timeForCheckSlider.value.ToString();
        }
            ...
            if (player1 != null && player2 != null)
            {
                if (isParamatersSetup)
                {
                    ...
                    if (player1.isPlayerReady && player2.isPlayerReady)
                    {
                        ...
                        GetComponent<FileWriter>().SetupFileWriter();
    ...
    //set the parameter based on the UI input
    public void SetupParameters()
    {
        maxTurnNum = (int)maxTurnNumSlider.value * 2;
        timeForEachTurn = (int)timeForEachTurnSlider.value;
        checkTime = (int)timeForCheckSlider.value;
    ...
    //dealy the parameter for 0.5 second to avoid the inputs from controller affect the next panel
    IEnumerator Delay()
    {
        yield return new WaitForSeconds(0.5f);

        isParamatersSetup = true;
    }
}
